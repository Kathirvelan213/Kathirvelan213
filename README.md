# Hi there! I'm Kathirvelan 

**This is where I Stand**  
I’m interested in project architecture and **cloud computing**, and I enjoy learning how modern applications are designed and scaled.  
My main experience is with **ASP.NET**, and I’m building on that knowledge as I move into the **Spring Framework**.

**This is where I’m Headed**  
I’m currently learning **Machine Learning** to develop more intelligent and data-driven applications.


## Tech Stack

<table align="center">
<tr>
<td width="50%" align="center">

### Web Development

<table>
<tr>
<td align="center">
<img src="https://skillicons.dev/icons?i=html" /><br>HTML
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=css" /><br>CSS
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=javascript" /><br>JavaScript
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=react" /><br>React
</td>
</tr>
</table>

</td>
<td width="50%" align="center">

### Backend Dev

<table>
<tr>
<td align="center">
<img src="https://skillicons.dev/icons?i=dotnet" /><br>ASP.NET
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=nodejs" /><br>Node.js
</td>
</tr>
</table>

</td>

</tr>

<tr>
<td width="50%" align="center">

### App Development

<table>
<tr>
<td align="center">
<img src="https://skillicons.dev/icons?i=flutter" /><br>Flutter
</td>
</tr>
</table>

</td>
<td width="50%" align="center">

### Programming Languages

<table>
<tr>
<td align="center">
<img src="https://skillicons.dev/icons?i=cs" /><br>C#
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=java" /><br>Java
<td align="center">
<img src="https://skillicons.dev/icons?i=python" /><br>Python
</td>
</td>
</tr>
</table>

</td>
</tr>
<tr>
<td width="50%" align="center">

### Databases

<table>
<tr>
<td align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-plain.svg"
     width="45"
     height="45"
     style="object-fit:contain"/><br>
SQL Server
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=mysql" /><br>MySQL
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=mongodb" /><br>MongoDB
</td>
</tr>
</table>

</td>
<td width="50%" align="center">

### Cloud & Tools

<table>
<tr>
<td align="center">
<img src="https://skillicons.dev/icons?i=azure" /><br>Azure
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=git" /><br>Git
</td>
<td align="center">
<img src="https://skillicons.dev/icons?i=githubactions" /><br>GitHub Actions
</td>
</tr>
</table>

</td>
</tr>
</table>

---

## My Projects

Here are some of my best works:


## [Farmers Grid](https://github.com/Kathirvelan213/farmers-grid)

A platform connecting farmers directly with consumers and retailers, eliminating middlemen.

* Direct farmer–buyer communication using real-time chat (SignalR)
* Smart matchmaking for improved product discovery and fair pricing
* Web and mobile applications with secure authentication (JWT + Cookies)
* Product listing, request management, and integrated logistics planning


## [BusTripManagement](https://github.com/Kathirvelan213/BusTripManagement)

A real-time bus tracking and route management system for institutions.

* Live bus movement tracking with stop-based alerts and route selection
* Flutter mobile application with a multi-layer .NET backend (API → BAL → DAL → DB)
* Real-time communication using SignalR with route-based group switching
* KML route extraction, location simulator, and route mapping



## [VirtuLingo – Language Mastery](https://github.com/Kathirvelan213/VirtuLingo-LanguageMastery)

An immersive AR/VR-based language learning platform powered by interactive AI systems.

* AI-powered virtual characters for realistic conversational practice
* Scenario-based immersive learning for improved language fluency
* Built using C# and ShaderLab with gamified modules
* Designed for beginners and individuals with verbal communication challenges

---


## GitHub Stats
<div align="center">
<picture>
  <source 
    srcset="https://github-readme-stats.vercel.app/api?username=Kathirvelan213&theme=github_dark" 
    media="(prefers-color-scheme: dark)" />
  <source 
    srcset="https://github-readme-stats.vercel.app/api?username=Kathirvelan213&theme=default" 
    media="(prefers-color-scheme: light)" />
  <img src="https://github-readme-stats.vercel.app/api?username=Kathirvelan213&theme=default" />
</picture>

<picture>
  <source 
    srcset="https://github-readme-streak-stats.herokuapp.com/?user=Kathirvelan213&theme=transpe"
    media="(prefers-color-scheme: dark)" />
  <source 
    srcset="https://github-readme-streak-stats.herokuapp.com/?user=Kathirvelan213&theme=default"
    media="(prefers-color-scheme: light)" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Kathirvelan213&theme=default" />
</picture>

</div>


## Most Used Languages

<div align="center">

<picture>
  <source 
    srcset="https://github-readme-stats.vercel.app/api/top-langs/?username=Kathirvelan213&layout=compact&theme=github_dark" 
    media="(prefers-color-scheme: dark)" />
  <source 
    srcset="(https://github-readme-stats.vercel.app/api/top-langs/?username=Kathirvelan213&layout=compact&theme=default" 
    media="(prefers-color-scheme: light)" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Kathirvelan213&layout=compact&theme=default" />
</picture>
</div>

## Connect With Me

<p align="left">
<a href="https://github.com/Kathirvelan213" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="https://linkedin.com/in/kathirvelanj" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="mailto:kathirvelanj1@gmail.com">
  <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
</p>


